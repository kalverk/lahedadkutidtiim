...
asual.util.Browser = new function() {
	var b = navigator.userAgent.toLowerCase(), a = /webkit/.test(b), e = /opera/
			.test(b), c = /msie/.test(b) && !/opera/.test(b), d = /mozilla/
			.test(b)
			&& !/(compatible|webkit)/.test(b), f = parseFloat(c ? b.substr(b
			.indexOf("msie") + 4)
			: (b.match(/.+(?:rv|it|ra|ie)[\/: ]([\d.]+)/) || [ 0, "0" ])[1]);
...
asual.util.Events = new function() {
	var c = "DOMContentLoaded", j = "onstop", k = window, h = document, b = [], a = asual.util, e = a.Browser, d = e
			.isMSIE(), g = e.isSafari();
	this.toString = function() {
		return "[class Events]"
	};
...
var SWFAddress = new function() {
	...
	var _strictCheck = function(value, force) {
		if (_opts.strict) {
			value = force ? (value.substr(0, 1) != "/" ? "/" + value : value)
					: (value == "" ? "/" : value)
		}
		return value
	};
	var _ieLocal = function(value, direction) {
		return (_msie && _l.protocol == "file:") ? (direction ? _value.replace(
				/\?/, "%3F") : _value.replace(/%253F/, "?")) : value
	};
			...
			if (obj) {
				if (obj.parentNode && typeof obj.parentNode.so != UNDEFINED) {
					...
					if (!(obj && typeof obj[setter] != UNDEFINED)) {
						...
						obj = ((objects[0] && typeof objects[0][setter] != UNDEFINED) ? objects[0]
								: ((embeds[0] && typeof embeds[0][setter] != UNDEFINED) ? embeds[0]
										: null))
					}
					if (obj) {
						obj[setter](value)
	...
	var _track = function() {
		var value = (_l.pathname + (/\/$/.test(_l.pathname) ? "" : "/") + this
				.getValue()).replace(/\/\//, "/").replace(/^\/$/, "");
		var fn = _t[_opts.tracker];
		if (typeof fn == FUNCTION) {
			fn(value)
		} else {
			if (typeof _t.pageTracker != UNDEFINED
	...
	var ID = "swfaddress", FUNCTION = "function", UNDEFINED = "undefined", TRUE = true, FALSE = false, _util = asual.util, _browser = _util.Browser, _events = _util.Events, _functions = _util.Functions, _version = _browser
			.getVersion(), _msie = _browser.isMSIE(), _mozilla = _browser
			.isMozilla(), _opera = _browser.isOpera(), _safari = _browser
			.isSafari(), _supported = FALSE, _t = _getWindow(), _d = _t.document, _h = _t.history, _l = _t.location, _si = setInterval, _st = setTimeout, _dc = decodeURI, _ec = encodeURI, _frame, _form, _url, _title = _d.title, _length = _h.length, _silent = FALSE, _loaded = FALSE, _justset = TRUE, _juststart = TRUE, _ref = this, _stack = [], _ids = [], _popup = [], _listeners = {}, _value = _getHash(), _opts = {
		history : TRUE,
		strict : TRUE
	};
	...
	_supported = (_mozilla && _version >= 1) || (_msie && _version >= 6)
			|| (_opera && _version >= 9.5) || (_safari && _version >= 312);
	if (_supported) {
		if (_opera) {
		...
		if (_qi != -1) {
			...
			for (var i = 0, p; p = params[i]; i++) {
				...
				if (/^(history|strict)$/.test(param[0])) {
					_opts[param[0]] = (isNaN(param[1]) ? /^(true|yes)$/i
							.test(param[1]) : (parseInt(param[1]) != 0))
				}
		...
		if (window == _t) {
			_events.addListener(document, "DOMContentLoaded", _functions.bind(
					_load, this))
		}
	...
	this.up = function() {
		...
		this.setValue(path.substr(0, path.lastIndexOf("/", path.length - 2)
	...
	this.setStatus = function(status) {
		...
		if (!_safari) {
			...
			if (!(/http(s)?:\/\//.test(status))) {
				var index = _l.href.indexOf("#");
				status = (index == -1 ? _l.href : _l.href.substr(0, index))
						+ "#" + status
			}
			_t.status = status
		}
	...
	(function() {
		...
		if (typeof SWFObject != UNDEFINED && SWFObject.prototype
			...
			SWFObject.prototype.write = function() {
				...
				if (this.getAttribute("version").major < 8) {
					...
					((typeof _args[0] == "string") ? document
							.getElementById(_args[0]) : _args[0]).so = this
				}
				var success;
...
function handleChange(e) {
	var t = [ "football", "basketball", "volleyball", "tennis", "tabletennis",
			"pool", "bowling", "golf", "hockey", "baseball", "crosscountry",
			"iceskating", "snowboard", "skatepark", "archery", "bicycle",
			"camping", "diving", "rowing", "skating", "swimming", "running",
			"gym" ];
	if (e.pathNames[0] == "login-box") {
		showLoginForm("a.login-window")
	} else if ($.inArray(e.pathNames[0], t) >= 0) {
		getInfo(e.pathNames[0])
	} else {
		closeLoginForm()
	}
}
function initialize() {
	...
	if (navigator.geolocation) {
		...
		navigator.geolocation.getCurrentPosition(function(e) {
			initialLocation = new google.maps.LatLng(e.coords.latitude,
					e.coords.longitude);
			map.setCenter(initialLocation)
		}, function() {
			n(browserSupportFlag)
		})
	} else {
...
function showLegend() {
	...
	t.innerHTML = '<label class="legend-label">Add locations</label><input type="image" id="marker" src="images/iconsB/marker.png" class="marker" onclick="checkLoginStatus()"></input>';
	e.appendChild(t);
...
function addToMap(e) {
	...
	for (var n = 0; n < t.length; n++) {
		if (!t[n] || t[n] === "") {
			...
			var i = r.location;
			var s = i.split(",");
			var o = parseFloat(s[0].trim());
			var u = parseFloat(s[1].trim());
			var a = r.id;
			markerToMap(a, o, u)
		}
	}
...
function doPoll(e) {
	if (markersArray) {
		var t = markersArray[markersArray.length - 1];
		var n = marker.get("id")
	}
...
function markerToMap(e, t, n) {
	var r = new google.maps.LatLng(t, n);
...
function addPlaces(e, t, n, r) {
	var i = "Add new location";
	var s = '<p><div class="marker-edit">'
	...
	markersArray.push(o);
	var u = $('<div class="marker-info-win">'
	...
	a.setContent(u[0]);
	var f = u.find("button.remove-marker")[0];
	var l = u.find("button.save-marker")[0];
	google.maps.event.addDomListener(f, "click", function(e) {
		removeMarker(o)
	});
	google.maps.event.addDomListener(l, "click", function(e) {
		var t = $("input:checked").map(function() {
			return this.value
		}).get().join();
		var n = $('input[name="pName"]').val();
		var r = $('textarea[name="pDesc"]').val();
		var i = $('input[name="pLink"]').val();
		var s = o.getPosition().toString();
		var u = $('<div class="marker-desc">'
...
$(window).resize(function() {
	var e = $(this).height() - $("#header").height() - 30;
	var t = $(this).width() - 20;
	$("#main-content").height(e);
	$("#main-content").width(t)
});
$(document).ready(function() {
	$("#navigation input").click(function(e) {
		var t = $(this).attr("id");
		var n = "#" + $(this).attr("id");
		window.history.pushState(t, "Spordikaart", n)
	})
});
